    ...
    run: async (bot, message, args) => {
        ...
        if (!user.presence.activities.length) {
            const sembed = new MessageEmbed()
                ...
                .addField(":negative_squared_cross_mark: **No Status**:negative_squared_cross_mark: ", 'This user does not have any custom status!')
        ...
        user.presence.activities.forEach((activity) => {
            ...
            if (activity.type === 'CUSTOM_STATUS') {
                const embed = new MessageEmbed()
                    .setAuthor(user.user.username, user.user.displayAvatarURL({ dynamic: true }))
                    .setColor("GREEN")
                    .addField(":white_check_mark: **Status** :white_check_mark: ", `**Custom status** -\n${activity.emoji || "No Emoji"} | ${activity.state}`)
                    .setThumbnail(user.user.displayAvatarURL())
                    .setFooter(message.guild.name, message.guild.iconURL())
                    .setFooter(`Super Store`, client.user.displayAvatarURL());
